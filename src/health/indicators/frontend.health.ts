...
export class FrontendHealthIndicator {
  private readonly logger = new Logger(FrontendHealthIndicator.name);
  ...
  async check(): Promise<ServiceHealth> {
    ...
    try {
      const url = this.configService.get<string>('FRONTEND_URL_HEALTH_CHECK');

      if (!url) {
        this.logger.error('FRONTEND_URL_HEALTH_CHECK is not configured');
        return {
          status: 'absent',
          responseTimeMs: 0,
        };
      ...
      if (axios.isAxiosError(error)) {
        if (error.code === 'ECONNREFUSED') {
          ...
          errorMessage = 'Timeout - service did not respond in a timely manner';
